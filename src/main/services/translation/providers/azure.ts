...
export function createAzureTranslatorProvider(): TranslationProvider {
  ...
  if (!key || !region) {
    throw new Error('Azure Translator requires AZURE_TRANSLATOR_KEY and AZURE_TRANSLATOR_REGION in .env.');
  }
  ...
  return {
    ...
    async translate(text: string): Promise<string> {
      ...
      if (!resp.ok) {
        ...
        throw new Error(`Azure Translator HTTP ${resp.status}: ${body}`);
      }
