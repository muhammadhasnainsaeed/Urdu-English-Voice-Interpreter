...
const api: ElectronAPI = {
  ...
  requestMicPermission: () => ipcRenderer.invoke('mic:request-permission') as Promise<PermissionStatus>,
  startStt: () => ipcRenderer.invoke('stt:start') as Promise<SttStartResult>,
  ...
  startTranslation: () => ipcRenderer.invoke('translation:start') as Promise<TranslationStartResult>,
  stopTranslation: () => ipcRenderer.invoke('translation:stop') as Promise<void>,
  onTranslationEvent: (handler: (event: TranslationEvent) => void) => {
    const listener = (_event: unknown, payload: TranslationEvent) => handler(payload);
    ipcRenderer.on('translation:event', listener);
    return () => {
      ipcRenderer.removeListener('translation:event', listener);
    };
  },
  startTts: () => ipcRenderer.invoke('tts:start') as Promise<TtsStartResult>,
  stopTts: () => ipcRenderer.invoke('tts:stop') as Promise<void>,
  onTtsEvent: (handler: (event: TtsEvent) => void) => {
    const listener = (_event: unknown, payload: TtsEvent) => handler(payload);
  ...
  startAudioOutput: () => ipcRenderer.invoke('audio-output:start') as Promise<AudioOutputStartResult>,
  stopAudioOutput: () => ipcRenderer.invoke('audio-output:stop') as Promise<void>,
  onAudioOutputEvent: (handler: (event: AudioOutputEvent) => void) => {
    const listener = (_event: unknown, payload: AudioOutputEvent) => handler(payload);
  ...
  detectBlackHole: () => ipcRenderer.invoke('audio-output:detect-blackhole') as Promise<boolean>,

  /* Session (Milestone 7) */
  startSession: () => ipcRenderer.invoke('session:start') as Promise<SessionStartResult>,
  stopSession: () => ipcRenderer.invoke('session:stop') as Promise<void>,
  onSessionEvent: (handler: (event: SessionEvent) => void) => {
    const listener = (_event: unknown, payload: SessionEvent) => handler(payload);
  ...
  onPipelineEvent: (handler: (event: PipelineEvent) => void) => {
    const listener = (_event: unknown, payload: PipelineEvent) => handler(payload);
