...
const ICON_STYLE: Record<SetupStepState, string> = {
...
function MicStep({
  ...
  switch (state.action) {
    case 'request-permission':
      ...
      break;
    case 'open-settings':
      detailText = 'Enable “Urdu English Interpreter” in System Settings → Privacy & Security → Microphone.';
      detail = (
        <div className="mt-2">
...
function OutputStep({
  ...
  else if (state.state === 'error') detail = 'No audio output available. Connect speakers or headphones.';
  else if (state.selectedDeviceLabel) detail = `Translations play through “${state.selectedDeviceLabel}”.`;
...
function BlackHoleStep({ state, onOpenSite }: { state: SetupState['blackhole']; onOpenSite: () => void }) {
