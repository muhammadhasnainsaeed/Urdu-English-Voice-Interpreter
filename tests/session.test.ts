...
import type { SessionEvent, TranslationEvent, TtsEvent, AudioChunk } from '../packages/shared/index';
    ...
    async fn() {
      ...
      try {
        ...
        if (!tts.lastSpokenText) {
          throw new Error('TTS never received translated text — session translation→TTS chain is broken');
        }
        if (tts.lastSpokenText !== 'Hello') {
          throw new Error(`Expected TTS to receive "Hello", got "${tts.lastSpokenText}"`);
        }
      } finally {
        mgr.stop();
        process.env.TRANSLATION_PROVIDER = '';
