...
import type { SessionEvent, TranslationEvent } from '../packages/shared/index';
...
function installFetch(control: FetchControl): void {
  globalThis.fetch = ((url: string | URL | Request): Promise<Response> => {
    ...
    const resp = control.queue.length > 1 ? control.queue.shift()! : control.queue[0];
    if (control.delayMs > 0) {
      return new Promise((resolve) => setTimeout(() => resolve(resp), control.delayMs));
    }
...
const tests: TestCase[] = [
...

  {
    ...
    async fn() {
      ...
      try {
        const provider = await makeProvider();
        const [a, b] = await Promise.all([provider.translate('same text'), provider.translate('same text')]);
